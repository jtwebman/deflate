...
We needed byte tracking for [SourceKeep](https://sourcekeep.dev) (a git hosting platform) to parse git pack files. We chose pure Elixir over a C/Rust NIF to avoid native dependencies and keep deployment simple - the performance difference is negligible for real-world workloads.
